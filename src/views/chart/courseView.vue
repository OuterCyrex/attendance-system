    ...
    <div class="course-chart-container">
        <div class="grid grid-cols-12 w-full">
            ...
            <ElCard class="col-span-12" shadow="never">
                <div class="mb-4 font-semibold text-xl">课程出勤情况图表</div>
            ...
            <ElCard class="col-span-12 mt-4" shadow="never" v-loading="chartLoading">
                <div class="flex">
                    <courseChart :data="classChartData" @change="handleGranularityChange" :loading="chartLoading" />
                </div>
            </ElCard>
        </div>
    </div>
...
import courseSelect from '@/components/select/courseSelect.vue'
import { fetchSemesterList } from '@/api/misc'
import { fetchGetCourseChart } from '@/api/chart'
...
const handleCourseSelect = (course: Api.Schedule.scheduleInfo) => {
    courseName.value = course.courseName
    orderNo.value = course.orderNo
}
...
const getClassChart = async () => {
    ...
    const data = await fetchGetCourseChart({
