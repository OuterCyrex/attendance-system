  ...
  <div class="page-content !border-0 !bg-transparent min-h-screen flex-cc">
    <div class="flex-cc max-md:!block max-md:text-center">
      ...
      <div class="ml-15 w-75 max-md:mx-auto max-md:mt-10 max-md:w-full max-md:text-center">
        ...
        <ElButton type="primary" size="large" @click="backHome" v-ripple class="mt-5 mr-8">{{
          data.btnText
        }}</ElButton>
        <ElButton type="info" size="large" @click="router.push('/auth/login')" v-ripple class="mt-5">重新登录</ElButton>
      </div>
    </div>
  </div>
</template>
