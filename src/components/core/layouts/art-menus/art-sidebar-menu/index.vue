...
<template>
  <div class="layout-sidebar" v-if="showLeftMenu || isDualMenu" :class="{ 'no-border': menuList.length === 0 }">
    <!-- 双列菜单（左侧） -->
    <div v-if="isDualMenu" class="dual-menu-left"
      ...
      <ElScrollbar style="height: calc(100% - 135px)">
        <ul>
          <li v-for="menu in firstLevelMenus" :key="menu.path" @click="handleMenuJump(menu, true)">
            <ElTooltip class="box-item" effect="dark" :content="$t(menu.meta.title)" placement="right" :offset="15"
              :hide-after="0" :disabled="dualMenuShowText">
              <div :class="{
                ...
                <ArtSvgIcon class="menu-icon text-g-700 dark:text-g-800" :icon="menu.meta.icon" :style="{
    ...
    <div v-show="menuList.length > 0" class="menu-left"
      ...
      <ElScrollbar :style="scrollbarStyle">
        ...
        <ElMenu :class="'el-menu-' + getMenuTheme.theme" :collapse="!menuOpen" :default-active="routerPath"
          :text-color="getMenuTheme.textColor" :unique-opened="uniqueOpened" :background-color="getMenuTheme.background"
          :default-openeds="defaultOpenedMenus"
          :popper-class="`menu-left-popper menu-left-${getMenuTheme.theme}-popper`" :show-timeout="50"
          :hide-timeout="50">
      ...
      <div class="menu-model" @click="toggleMenuVisibility" :style="{
        opacity: !menuOpen ? 0 : 1,
...
const hasPermission = (item: any) => {
  // 1. 如果菜单没配置 roles，或者是空数组，说明所有人都能看
  if (!item.meta || !item.meta.roles || item.meta.roles.length === 0) {
    return true
  }
  // 2. 获取当前用户角色 (确保你的 userStore 里有 role 字段)
...
// 核心：递归过滤整个菜单树
const filterMenusByRole = (menus: any[]) => {
  const res: any[] = []

  menus.forEach(menu => {
    // 深拷贝，避免直接修改 Store 里的数据
    const tmp = { ...menu }
    ...
    if (hasPermission(tmp)) {
      ...
      if (tmp.children && tmp.children.length > 0) {
        tmp.children = filterMenusByRole(tmp.children)

        // (可选优化) 如果子菜单过滤完变空了，且父菜单本身不承载页面，可能需要隐藏父菜单
...
const firstLevelMenus = computed(() => {
  ...
  // 先按权限过滤，再按 isHide 过滤
  const permissionMenus = filterMenusByRole((allMenus))
  return permissionMenus.filter((menu) => !menu.meta.isHide)
})

const menuList = computed(() => {
  const menuStore = useMenuStore()
  const allMenus = filterMenusByRole(menuStore.menuList)
